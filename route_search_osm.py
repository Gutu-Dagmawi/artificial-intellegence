...
class RouteSearchSystem:
    ...
    def __init__(self, place_name: str = "Addis Ababa, Ethiopia"):
        ...
        try:
            ...
            print(f"✓ Successfully loaded {len(self.graph_unprojected.nodes)} nodes and {len(self.graph_unprojected.edges)} edges")
            ...
            print(f"✓ Graph projected to UTM for accurate distance calculations")
    ...
    def display_available_locations(self):
        ...
        if not places:
            print("\n⚠️  Could not extract named locations from the graph.")
        ...
        print(f"\n✓ Found {len(places)} named locations in the map:")
        print()
    ...
    def geocode_location(self, location: str, verbose: bool = True) -> Optional[int]:
        ...
        try:
            ...
            if distance > 1000:  # More than 1km away
                print(f"   ⚠️  Warning: Nearest road is {distance:.0f}m ({distance/1000:.1f}km) from geocoded location")
    ...
    def uniform_cost_search(self, start: int, goal: int) -> Tuple[Optional[List[int]], Dict]:
        """
    ...
    def search_route(self, start_location: str, goal_location: str) -> Dict:
        ...
        if start_node is None:
            print(f"\n❌ Invalid input: location not found - '{start_location}'")
            return {'error': 'start_not_found'}
        
        if goal_node is None:
            print(f"\n❌ Invalid input: location not found - '{goal_location}'")
        ...
        print(f"\n📊 Route Overview:")
        print(f"   Direct distance: {direct_distance:.1f}m ({direct_distance/1000:.2f}km)")
        ...
        if start_node == goal_node:
            ...
            if start_location.lower().strip() != goal_location.lower().strip():
                print(f"\n⚠️  Warning: '{start_location}' and '{goal_location}' geocoded to the same location!")
                print(f"   This may mean:")
                print(f"   • The locations are very close together")
                print(f"   • The location names are too general")
                print(f"   • Try being more specific (e.g., add street names or landmarks)")
        ...
        for algo_name, data in results.items():
            print(f"\n{algo_name}:")
            print(f"   Distance:        {data['distance']:.2f} meters ({data['distance']/1000:.2f} km)")
        ...
        optimal_algorithms = [
            algo for algo, data in results.items() 
            if abs(data['distance'] - min_distance) < 0.01  # Account for floating point
        ]
        
        print("\n" + "-"*70)
        print(f"🏆 Optimal path distance: {min_distance:.2f} meters ({min_distance/1000:.2f} km)")
        
        if len(optimal_algorithms) > 1:
            print(f"⚠️  Multiple optimal paths found!")
        ...
        for algo_name, data in results.items():
            efficiency = "✓ OPTIMAL" if algo_name in optimal_algorithms else "  Sub-optimal"
            print(f"{algo_name:8} - {efficiency:15} | Explored: {data['nodes_explored']:6} nodes")
    ...
    def visualize_route(self, path: List[int], algorithm_name: str = "Route", 
                       start_name: str = "Start", goal_name: str = "Goal",
                       distance_km: float = 0):
        """
        ...
        try:
            ...
            for i in range(waypoint_interval, len(path) - 1, waypoint_interval):
                ...
                # Add label for waypoint with street name (only if readable/Latin script)
                if street_name != "Unnamed Road" and self._is_latin_script(street_name):
            ...
            legend_elements = [
                Line2D([0], [0], marker='o', color='w', markerfacecolor='#28a745', 
                      markersize=12, label='Start', markeredgecolor='white', markeredgewidth=2),
                Line2D([0], [0], marker='*', color='w', markerfacecolor='#dc3545', 
                      markersize=14, label='Destination', markeredgecolor='white', markeredgewidth=2),
                Line2D([0], [0], marker='o', color='w', markerfacecolor='#ffc107', 
                      markersize=8, label='Waypoints', markeredgecolor='white', markeredgewidth=1.5),
            ...
            print("   💡 Use your mouse to zoom and pan the map")
            print("   💡 Use the toolbar buttons for more controls")
...
def main():
    ...
    try:
        ...
        print("\n📍 Would you like to see available locations from the map? (y/n): ", end="")
        ...
        goal_location = input("🎯 Destination: ").strip()
        ...
        print("\n" + "="*70)
        visualize = input("\n📊 Would you like to visualize the optimal route? (y/n): ").strip().lower()
        ...
        print("\n✅ Route search completed successfully!")
